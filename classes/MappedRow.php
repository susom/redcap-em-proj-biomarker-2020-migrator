...
and rd.value = '%s'",
    ...
    function mapRow($row) {
        ...
        foreach ($row as $key => $val) {
            ...
            //==================HANDLE BAD CODE==========================//
/**
            if( ($mapper[$key]['to_field'] == 'moyr_pg1') OR ($mapper[$key]['to_field'] == 'moyr_pg2')){
                $module->emDebug("VAL: ". $val);
                $val = DateTime::createFromFormat("M-y", $val)->format("m/Y"); //Incoming is "Nov-80", desired output is 11/1980
 ...
 */
            ...
            if( ($mapper[$key]['to_field'] == 'moyr_c1') OR ($mapper[$key]['to_field'] == 'moyr_c2')){
                //2017_29 has a different incoming format than 2020_21
                    $module->emDebug("===2020 version =PRE: " . $val);

                    $pieces = explode("-", $val);
                    $val = $pieces[0] . "-" . $pieces[1];  //incoming is mm/dd/yyyy; desired format is 2018-06
